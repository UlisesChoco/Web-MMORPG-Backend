...
export class NpcItemController implements OnModuleInit {
  ...
  constructor(
    @Inject('NPC_ITEM_PACKAGE') private readonly client: ClientGrpc,
  ) {}

  onModuleInit() {
    this.npcItemService =
      this.client.getService<NPCItemService>('NPCItemService');
  }
  ...
  async listNPCVendorItems(
    ...
    try {
      ...
      this.logger.log(
        `Items del NPC con ID ${params.npcId} listados exitosamente`,
      );

      return finalData;
    } catch (err) {
      this.logger.error(
        `Error al listar los items del NPC con ID ${params.npcId}: ${err.message}`,
      );
