...
export class MapController implements OnModuleInit {
  ...
  async getMapById(
    ...
    try {
      ...
      this.logger.error(
        `Error al obtener el mapa con id ${params.id}: ${err.message}`,
      );
  ...
  @Get()
  async listMaps(@Res({ passthrough: true }) response: Response) {
    try {
      const request = {};
  ...
  async listMapEnemies(
    ...
    try {
      ...
      this.logger.log(
        `Lista de enemigos del mapa con id ${params.id} obtenida exitosamente`,
      );

      return finalData;
    } catch (err) {
      this.logger.error(
        `Error al obtener la lista de enemigos del mapa con id ${params.id}: ${err.message}`,
      );
  ...
  async getExpandedMapEnemyPool(
    ...
    try {
      ...
      this.logger.log(
        `Pool de enemigos expandido del mapa con id ${params.id} obtenido exitosamente`,
      );

      return finalData;
    } catch (err) {
      this.logger.error(
        `Error al obtener el pool de enemigos expandido del mapa con id ${params.id}: ${err.message}`,
      );
