...
export class FatalCombatReplayController implements OnModuleInit {
  ...
  constructor(
    @Inject('FATAL_COMBAT_REPLAY_PACKAGE') private readonly client: ClientGrpc,
  ) {}

  onModuleInit() {
    this.fatalCombatReplayService =
      this.client.getService<FatalCombatReplayService>(
        'FatalCombatReplayService',
      );
  }
  ...
  async getFatalCombatReplay(
    ...
    try {
      ...
      const observable =
        this.fatalCombatReplayService.GetFatalCombatReplay(request);
      ...
      this.logger.error(
        `Error al obtener replay de combate fatal con ID: ${params.id}: ${err}`,
      );
  ...
  async getRecentFatalities(
    ...
    try {
      ...
      const observable =
        this.fatalCombatReplayService.GetRecentFatalities(request);
