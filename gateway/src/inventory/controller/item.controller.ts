...
export class ItemController implements OnModuleInit {
  ...
  async getItemsByRequiredLevel(
    ...
    try {
      ...
      this.logger.error(
        `Error al obtener items por nivel requerido: ${err.message}`,
      );
  ...
  async getItemsByRequiredLevelAndSlot(
    ...
    try {
      ...
      const observable =
        this.itemService.GetItemsByRequiredLevelAndSlot(request);

      const data = await firstValueFrom(observable);

      const finalData =
        ItemMapper.toGetItemsByRequiredLevelAndSlotResponseDTO(data);

      this.logger.log(
        `Items obtenidos exitosamente por nivel requerido y slot`,
      );

      return finalData;
    } catch (err) {
      this.logger.error(
        `Error al obtener items por nivel requerido y slot: ${err.message}`,
      );
