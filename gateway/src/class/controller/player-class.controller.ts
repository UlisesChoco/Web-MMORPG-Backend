...
export class PlayerClassController implements OnModuleInit {
  ...
  constructor(
    @Inject('PLAYER_CLASS_PACKAGE') private readonly client: ClientGrpc,
  ) {}

  onModuleInit() {
    this.playerClassService =
      this.client.getService<PlayerClassService>('PlayerClassService');
  }
  ...
  async getScaledClassStats(
    ...
    try {
      ...
      this.logger.log(
        `Devolviendo las estadísticas escaladas de la clase con ID: ${params.id}`,
      );

      return data;
    } catch (err) {
      this.logger.error(
        `Error al obtener las estadísticas escaladas de la clase con ID: ${params.id}`,
        err,
      );
  ...
  @Get()
  @HttpCode(HttpStatus.OK)
  async listClasses(@Res({ passthrough: true }) res: Response) {
    try {
      const request = {};
