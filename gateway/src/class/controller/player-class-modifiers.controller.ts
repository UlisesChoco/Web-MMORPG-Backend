...
export class PlayerClassModifiersController implements OnModuleInit {
  ...
  constructor(
    @Inject('PLAYER_CLASS_MODIFIER_PACKAGE')
    private readonly client: ClientGrpc,
  ) {}

  onModuleInit() {
    this.playerClassModifierService =
      this.client.getService<PlayerClassModifierService>(
        'PlayerClassModifierService',
      );
  }
  ...
  async getClassModifiers(
    ...
    try {
      ...
      const observable =
        this.playerClassModifierService.GetClassModifiers(request);

      const data = await firstValueFrom(observable);

      this.logger.log(
        `Modificadores de clase obtenidos para la clase con ID ${params.id}`,
      );

      return data;
    } catch (err) {
      this.logger.error(
        `Error al obtener los modificadores de clase para la clase con ID ${params.id}: ${err}`,
      );
