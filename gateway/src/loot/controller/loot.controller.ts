...
export class LootController implements OnModuleInit {
  ...
  async getEnemyDropTable(
    ...
    try {
      ...
      this.logger.log(
        `Tabla de drops del enemigo con ID ${params.enemyId} obtenida exitosamente.`,
      );

      return data;
    } catch (err) {
      this.logger.error(
        `Error al obtener la tabla de drops del enemigo con ID ${params.enemyId}: ${err.message}`,
      );
