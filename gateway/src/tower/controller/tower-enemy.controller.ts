...
export class TowerEnemyController implements OnModuleInit {
  ...
  constructor(
    @Inject('TOWER_ENEMY_PACKAGE') private readonly client: ClientGrpc,
  ) {}

  onModuleInit() {
    this.towerEnemyService =
      this.client.getService<TowerEnemyService>('TowerEnemyService');
  }
  ...
  async listTowerEnemies(
    ...
    try {
      ...
      this.logger.log(
        `Enemigos de torre listados exitosamente para torreId: ${params.id}`,
      );
