...
export class TowerController implements OnModuleInit {
  ...
  async getTowerFloor(
    ...
    try {
      ...
      this.logger.log(
        `Piso de la torre con id ${params.id} obtenido exitosamente`,
      );

      return finalData;
    } catch (err) {
      this.logger.error(
        `Error al obtener el piso de la torre con id ${params.id}: ${err.message}`,
      );
  ...
  @Get()
  async listTowerFloors(@Res({ passthrough: true }) response: Response) {
    try {
      const request = {};
      ...
      this.logger.error(
        `Error al obtener los pisos de la torre: ${err.message}`,
      );
