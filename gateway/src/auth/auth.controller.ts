...
export class AuthController implements OnModuleInit {
  ...
  async login(
    ...
    try {
      ...
      this.logger.log(
        `Usuario ${loginDTO.email} loggeado correctamente, su cookie JWT expirará en ${expiresAt.toISOString()}`,
      );

      return data;
    } catch (err) {
      this.logger.error(
        `Error al loggear el usuario ${loginDTO.email}: ${err}`,
      );
  ...
  async register(
    ...
    try {
      ...
      this.logger.error(
        `Error al registrar el usuario ${registerDTO.email}: ${err}`,
      );
